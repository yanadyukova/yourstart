...
export function gitCommit() {
    return gulp.src('./git/*')
        ...
        .pipe(plugins.git.commit('Frontend source').on('error', function(err) {
            gutil.beep();
            if (err) throw err;
        }))
        .pipe(plugins.git.add({
        ...
        .pipe(plugins.git.commit('Frontend build').on('error', function(err) {
            gutil.beep();
            if (err) throw err;
        }))
        .pipe(plugins.git.add({
        ...
        .pipe(plugins.git.commit('Frontend update cache').on('error', function(err) {
            gutil.beep();
            if (err) throw err;
        }));
}
